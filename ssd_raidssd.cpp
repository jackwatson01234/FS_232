...
double RaidSsd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, char data)
{
	return event_arrive(type, logical_address, size, start_time, NULL, data);
}
...
//data added az a inut 
double RaidSsd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, void *buffer, char data)
{
	// printf("----------------------------------------------------------------- %c\n", data);
	if (type == WRITE)
	...
	if (PARALLELISM_MODE == 1) // Striping
	{
		...
		for (int i=0;i<RAID_NUMBER_OF_PHYSICAL_SSDS;i++)
		{
			if (buffer == NULL)
			{
				...
				timings[i] = Ssds[i].event_arrive(type, logical_address, size, start_time, NULL, data);
			}

			else
				timings[i] = Ssds[i].event_arrive(type, logical_address, size, start_time, (char*)buffer +(i*PAGE_SIZE), data);
	...
	{
		return Ssds[logical_address%RAID_NUMBER_OF_PHYSICAL_SSDS].event_arrive(type, logical_address, size, start_time, (char*)buffer, data);
	}
