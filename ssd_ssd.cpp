...
// add data az input
double Ssd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, char data)
{
...
// add data az input
double Ssd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, void *buffer, char data)
{
	...
	if((event = new Event(type, logical_address , size, start_time, data)) == NULL)
	{
...
void Ssd::print_statistics()
{
	...
	controller.MTStats.MT_print_statistics(); 
}
