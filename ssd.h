...
public:
	...
	MT_Stats(void);
	// print stats of Mapping Table
	void MT_print_statistics();
	// reset all of variable in MT_Stats
	void MT_reset_statistics();
private:
	void MT_reset();
};
...
{
public:
	Event(enum event_type type, ulong logical_address, uint size, double start_time, char data);
	~Event(void);
	//---------------------------------
...
private:
	//////////////////////////////////////////////////////////
	enum status issueBusLock(Event &event_list, double time);
	enum status issueRamRead(Event &event_list);
	enum status issueRamWrite(Event &event_list);
	enum status issueRead(Event &event_list);
	enum status issueWrite(Event &event_list);	
	//////////////////////////////////////////////////////////	
...
public:
	...
	~Ssd(void);
	double event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, char data);
	double event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, void *buffer, char data);
