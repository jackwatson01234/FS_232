...
//////////////////////////////////////////////////////////////////////////////
enum status Controller::issueBusLock(Event &event_list, double time)
{
	Event *cur;
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		...
		else if (ssd.bus.lock(cur -> get_address().package, cur -> get_start_time(), time, *cur) == FAILURE)
		{
			return FAILURE;
		}
	}	
...
enum status Controller::issueRamRead(Event &event_list)
{
	Event *cur;
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		assert(cur -> get_address().valid > NONE);
		...
		else if (ssd.ram.read(*cur) == FAILURE)
		{
			return FAILURE;
		}
	}
...
enum status Controller::issueRamWrite(Event &event_list)
{
	Event *cur;
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		assert(cur -> get_address().valid > NONE);
		...
		else if (ssd.ram.write(*cur) == FAILURE)
		{
			return FAILURE;
		}
	}
...
enum status Controller::issueRead(Event &event_list)
{
	Event *cur;
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		assert(cur -> get_address().valid > NONE);
...
enum status Controller::issueWrite(Event &event_list)
{
	Event *cur;
	for(cur = &event_list; cur != NULL; cur = cur -> get_next()){
		assert(cur -> get_address().valid > NONE);
